export const items = [
  {
    titulo: "Bíblia Slim Filament Marrom - NVT",
    autor: "Mundo Cristão",
    imagem:
      "https://images.tcdn.com.br/img/img_prod/400550/pre_venda_biblia_slim_filament_preta_nvt_2061_2_af82f09586aacb524dfa755e81ce3ec3.jpg",
    preco: 165,
    descricao:
      "A Bíblia Filament é a versão mais inovadora que já foi publicada em língua portuguesa. Ela une a simplicidade e clareza da Nova Versão Transformadora (NVT) com uma experiência digital imersiva que vai surpreender você! Além do texto impresso, a Bíblia oferece acesso gratuito ao aplicativo Filament, que é fácil de utilizar e expande sua leitura com recursos incríveis, diretamente no seu celular ou tablet.",
  },
  {
    titulo: "Bíblia Slim Filament Preta - NVT",
    autor: "Mundo Cristão",
    imagem:
      "https://images.tcdn.com.br/img/img_prod/400550/biblia_365_naa_edicao_limitada_2071_1_13e6ec29c90c9550d5de5d647f6f3c43.jpg",
    preco: 165,
    descricao:
      "A Bíblia Filament é a versão mais inovadora que já foi publicada em língua portuguesa. Ela une a simplicidade e clareza da Nova Versão Transformadora (NVT) com uma experiência digital imersiva que vai surpreender você! Além do texto impresso, a Bíblia oferece acesso gratuito ao aplicativo Filament, que é fácil de utilizar e expande sua leitura com recursos incríveis, diretamente no seu celular ou tablet.",
  },
  {
    titulo: "Bíblia 365 NAA - Edição Limitada",
    autor: "Mundo Cristão",
    imagem:
      "https://images.tcdn.com.br/img/img_prod/400550/pre_venda_biblia_slim_filament_marrom_nvt_2059_2_6b31a81586efbab2051f46e7cb748571.jpg",
    preco: 70,
    descricao:
      "A Bíblia 365 chegou em uma versão exclusiva e ainda mais especial! Além de oferecer um plano de leitura diário cuidadosamente elaborado, que combina porções do Antigo Testamento, Novo Testamento, Salmos e Provérbios, esta edição traz um toque único: uma jaqueta personalizada que transforma o produto em uma verdadeira obra de arte.",
  },
  {
    titulo: "Box das Crianças",
    autor: "JesusCopy",
    imagem:
      "https://images.tcdn.com.br/img/img_prod/400550/box_dia_das_criancas_2037_1_ef071c6ab942531f4bd19842274c3b97.jpg",
    preco: 90,
    descricao:
      "A Bíblia comentada Kids é o novo projeto do JesusCopy que nós desenvolvemos com o objetivo de dar ferramenta para os pais criarem uma paixão em seus filhos pela Bíblia. Ela contém 50 capítulos com 50 conteúdos e a cada conteúdo trazemos um versículo, uma reflexão, uma oração e uma atividade lúdica para que você possa fazer junto com seu filho.",
  },
];
